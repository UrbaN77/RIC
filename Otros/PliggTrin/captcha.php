            ...
            )
{
	...
	{
		if ($is_verbose) echo "archivo $filename no encontrado\n";
		return false;
	}
    $postdata = array(
        'method'    => 'post', 
    ...
    {
    	if ($is_verbose) echo "Error con el captcha: $result\n";
        return false;
    }
    else
    {
        $ex = explode("|", $result);
        $captcha_id = $ex[1];
    	if ($is_verbose) echo "Captcha enviado, ID $captcha_id\n";
        $waittime = 0;
        if ($is_verbose) echo "esperando por $rtimeout segundos\n";
        sleep($rtimeout);
        while(true)
        {
            ...
            if (strpos($result, 'ERROR')!==false)
            {
            	if ($is_verbose) echo "El servidor retorno un error: $result\n";
                return false;
            }
            if ($result=="CAPCHA_NOT_READY")
            {
            	if ($is_verbose) echo "captcha no listo aun\n";
            	$waittime += $rtimeout;
